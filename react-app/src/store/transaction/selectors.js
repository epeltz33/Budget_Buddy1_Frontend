...
export const selectTransactionsByDate = createSelector(
  ...
  (transactions, dateQuery) => {
    ...
    const firstDateMonth = isRange
      ? parseInt(dateQuery.split("&")[0].splice(4, 6))
      : parseInt(dateQuery.splice(4, 6)); // get month from dateQuery
    const firstDateYear = isRange
      ? parseInt(dateQuery.split("&")[0].splice(0, 4))
      : parseInt(dateQuery.splice(0, 4)); // get year from dateQuery
    const firstDateDay = isRange
      ? parseInt(dateQuery.split("&")[0].splice(6))
      : parseInt(dateQuery.splice(6)); // get day from dateQuery

    const secondDateMonth = isRange
      ? parseInt(dateQuery.split("&")[1].splice(4, 6))
      : null;
    const secondDateYear = isRange
      ? parseInt(dateQuery.split("&")[1].splice(0, 4))
      : null;
    const secondDateDay = isRange
      ? parseInt(dateQuery.split("&")[1].splice(6))
      : null;

    const firstDate = new Date(
      Date.UTC(firstDateYear, firstDateMonth - 1, firstDateDay)
    ); // create date object from dateQuery
    const secondDate = isRange
      ? new Date(Date.UTC(secondDateYear, secondDateMonth - 1, secondDateDay))
      : null;

    const transaction_date = isRange
      ? transactions.filter(
          (transaction) =>
            new Date(transaction.date) >= firstDate &&
            new Date(transaction.date) <= secondDate
        )
      : transactions.filter(
          (transaction) =>
            new Date(transaction.date).getMonth() === firstDate.getMonth() &&
            new Date(transaction.date).getFullYear() === firstDate.getFullYear()
        );
...
export const selectTransactionsByPayee = createSelector(
  [transactions, (state, payeeId) => payeeId],
...
export const selectTransactionByAmount = createSelector(
  ...
  (transactions, amountQuery) => {
    return transactions.filter(
      (transaction) => transaction.amount === +amountQuery
    );
  }
);
